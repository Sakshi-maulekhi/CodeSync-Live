...
const LoginForm = ({ isOpen }) => {
  ...
  async function formAction({ request }) {
    ...
    // 🔥 FIX: send username instead of name
    if (signup) {
      authData.name = request.name;
    }

    const currentMode = signup ? "register" : "login";

    try {
      setLoading(true);

      const response = await fetch(
        `${serverUrl}/api/auth/${currentMode}`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        const errorMsg =
          respData?.message ||
          respData?.error?.email ||
          respData?.error?.password ||
...
if (token) {
  setLoggedIn(true);
  saveAuthToken(token, respData.user.username, respData.user.email);
