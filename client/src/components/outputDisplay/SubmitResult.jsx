...
const SubmitResult = () => {
  const { submitResponse, submitResult } = useContext(CodeEditorContext);
  const [showAccepted, setShowAccepted] = useState(false);

  // Show accepted dialog when status is Accepted
  useEffect(() => {
  ...
  return (
    <>
        ...
        {submitResponse ? (
          ...
          <>
            {submitResult.status === "Accepted" ? (
              // LeetCode-style ACCEPTED display
              ...
              // LeetCode-style WRONG ANSWER display
              <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded">
                <div className="font-bold text-lg mb-3">✗ Wrong Answer</div>
                
                <div className="text-sm mb-3">
                  Test case #{submitResult.failedTestCase?.index} / {submitResult.totalTestCases}
                </div>
      ...
      <SubmitReport
        accepted={submitResult?.status === "Accepted"}
